...
string getShell()
{
  string shellPath = getenv("SHELL");

  stringstream pathStream(shellPath);

  string shell;

  while (getline(pathStream, shell, '/'))
  {
    continue;
  }
  return shell;
...
int main() {
  ...
  string os =
      exec("grep PRETTY_NAME /etc/os-release | cut -d '=' -f 2 | tr -d '\"'");
  ...
  cout << WHITE << "⠀⠀⢰⣜⠘⡀⢡⠰⠳⣎⢂⣟⡎⠘⣬⡕⣈⣼⠢⠹⡟⠇⠀     " << BLUE << BOLD     << "Host: " << RESET << WHITE    << exec("cat /sys/devices/virtual/dmi/id/product_name 2>/dev/null || "        "echo unknown")       << " "       << exec("cat /sys/devices/virtual/dmi/id/product_version 2>/dev/null || "  "echo unknown")       << RESET << endl;
